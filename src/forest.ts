...
export class Forest extends Ecs.World {
  ...
  detach(
    ...
    if (fork.left.is_window(window)) {
      if (parent && fork.right) {
        const pfork = this.reassign_child_to_parent(
          fork_entity,
          parent,
          fork.right,
        );
        ...
        switch (fork.right.inner.kind) {
          case 1:
            this.reassign_children_to_parent(
              fork_entity,
              (fork.right.inner as Node.NodeFork).entity,
              fork,
            );
            break;
          default:
  ...
  private grow_sibling(
    ...
    const resize_fork = () =>
      this.resize_fork_(ext, fork_e, crect, movement, false);
    ...
    if (fork_c.is_horizontal()) {
      if ((movement & (DOWN | UP)) != 0) {
        ...
        this.readjust_fork_ratio_by_right(
          ext,
          crect.width,
          fork_c,
          fork_c.area.width,
  ...
  *iter(
    entity: Entity,
    kind: Node.NodeKind | null = null,
  ): IterableIterator<Node.Node> {
  ...
  resize(
    ...
    const is_left =
      fork_c.left.is_window(win_e) || fork_c.left.is_in_stack(win_e);

    ((movement & Movement.SHRINK) != 0
      ? this.shrink_sibling
      : this.grow_sibling
   ...
   */
  private reassign_child_to_parent(
    child_entity: Entity,
   ...
   */
  private reassign_children_to_parent(
    parent_entity: Entity,
    child_entity: Entity,
    p_fork: Fork.Fork,
  ) {
   ...
   */
  private readjust_fork_ratio_by_right(
    ext: Ext,
    right_length: number,
    fork: Fork.Fork,
    fork_length: number,
        ...
        if ((mov & UP) !== 0) {
          if (shrunk) {
            if (
              ...
              src_node.area.y + src_node.area.height
            ) {
              break;
            }
          ...
          if (shrunk) {
            if (
              ...
              src_node.area.x + src_node.area.width
            ) {
              break;
            }
          } else if (!child.is_horizontal() || !is_left) {
  ...
  private shrink_sibling(
    ...
    const resize_fork = () =>
      this.resize_fork_(ext, fork_e, crect, movement, true);
  ...
  private display_branch(ext: Ext, branch: Node.Node, scope: number): string {
    switch (branch.inner.kind) {
      case 1:
        ...
        return fork
          ? this.display_fork(ext, branch.inner.entity, fork, scope + 1)
          : "Missing Fork";
      case 2:
  ...
  display_fork(
    ...
    fmt +=
      " ".repeat((1 + scope) * 2) +
      "left: " +
      this.display_branch(ext, fork.left, scope) +
      ",\n";
    fmt +=
      " ".repeat((1 + scope) * 2) +
      "parent: " +
      this.parents.get(fork.entity) +
      ",\n";
    ...
    if (fork.right) {
      fmt +=
        " ".repeat((1 + scope) * 2) +
        "right: " +
        this.display_branch(ext, fork.right, scope) +
        ",\n";
    }
...
function move_window(
  ...
  if (!(window.meta instanceof Meta.Window)) {
    log.error(
      `attempting to a window entity in a tree which lacks a Meta.Window`,
    );
    return;
  }
  ...
  if (!actor) {
    log.warn(
      `Window(${window.entity}) does not have an actor, and therefore cannot be moved`,
    );
    return;
  }
