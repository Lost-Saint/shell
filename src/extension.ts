...
export class Ext extends Ecs.System<ExtEvent> {
      ...
      const generator = (stdout: any, res: any) => {
        try {
          ...
          if (bytes) {
            if (
              event_handler(
                (imports.byteArray.toString(bytes) as string).trim(),
              )
            ) {
  ...
  focus_right() {
    this.stack_select(
      (id, stack) =>
        stack.tabs.length > id + 1 ? stack.tabs[id + 1].entity : null,
    ...
    if (prev_area && next_area) {
      ...
      if (this.auto_tiler) {
        if (this.is_floating(win)) {
          win.meta.set_unmaximize_flags(Meta.MaximizeFlags.HORIZONTAL);
          win.meta.set_unmaximize_flags(Meta.MaximizeFlags.VERTICAL);
          win.meta.unmaximize();
        }
  ...
  move_workspace(direction: Meta.DisplayDirection) {
    ...
    const workspace_move = (direction: Meta.MotionDirection) => {
      ...
      const last_window = (): boolean => {
        const last =
          wom.get_n_workspaces() - 2 === ws.index() && ws.n_windows === 1;
        return last;
      };
    ...
    if (win) {
      ...
      if (win.is_tilable(this)) {
        ...
        this.drag_signal = GLib.timeout_add(GLib.PRIORITY_LOW, 200, () => {
          ...
          let windowless =
            this.auto_tiler.largest_on_workspace(this, monitor, workspace) ===
            null;
          ...
          if (windowless) {
            ...
            const is_sibling = this.auto_tiler.windows_are_siblings(
              entity,
              attach_to.entity,
            );
        ...
        this.register_fn(() => {
          ...
          if (meta_window) {
            ...
            if (shell_window) {
              ...
              if (
                this.auto_tiler &&
                meta_window.window_type === Meta.WindowType.DESKTOP
              ) {
  ...
  *tiled_windows(): IterableIterator<Entity> {
  ...
  update_display_configuration(workareas_only: boolean) {
    ...
    const primary_display_ready = (ext: Ext): boolean => {
      ...
      return !(
        area.width === work_area.width && area.height === work_area.height
      );
    };
    ...
    for (const [entity, w] of this.windows.iter()) {
      ...
      this.monitors.with(entity, ([mon]) => {
        const assignment =
          mon === old_primary ? primary_display : w.meta.get_monitor();
